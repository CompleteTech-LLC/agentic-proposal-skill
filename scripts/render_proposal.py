#!/usr/bin/env python3
"""List and render agentic development proposal templates."""
...
CATALOG_MD = ROOT / "references" / "proposal-catalog.md"
...
def extract_template(template_id: str) -> str:
    ...
    if not match:
        raise KeyError(f"Template not found in proposal-catalog.md: {template_id}")
...
def main() -> int:
    ...
    parser.add_argument("--list", action="store_true", help="List available proposal template IDs.")
    parser.add_argument("--stage", help="Filter --list by stage.")
    parser.add_argument("--type", dest="proposal_type", help="Filter --list by type.")
    ...
    if args.list:
        for item in templates:
            if args.stage and item["stage"] != args.stage:
                continue
            if args.proposal_type and item["type"] != args.proposal_type:
                continue
